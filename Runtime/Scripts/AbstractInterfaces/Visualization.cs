...
namespace AbstractInterfaces
{
	public abstract class Visualization : MonoBehaviour
	{
		...
		private NativeArray<float3x4> m_positions, m_normals;

		private ComputeBuffer m_positionsBuffer, m_normalsBuffer;
		...
		private void Update()
		{
			if (m_isDirty || transform.hasChanged)
			{
				...
				UpdateVisualization(m_positions, resolution,
									ShapeJobs[(int) shape](m_positions, m_normals, resolution, transform.localToWorldMatrix, default));
