	...
	public static class Shapes
	{
		...
		public struct Plane : IShape
		{
			public Point4 GetPoint4(int i, float resolution, float invResolution)
			{
				...
				return new Point4 {positions = float4x3(uv.c0 - 0.5f, 0f, uv.c1 - 0.5f), normals = float4x3(0f, 1f, 0f)};
			}
		}
		...
		public struct Sphere : IShape
		{
			public Point4 GetPoint4(int i, float resolution, float invResolution)
			{
				...
				float4 scale = 0.5f
							 * rsqrt(p.positions.c0 * p.positions.c0 + p.positions.c1 * p.positions.c1 + p.positions.c2 * p.positions.c2);
